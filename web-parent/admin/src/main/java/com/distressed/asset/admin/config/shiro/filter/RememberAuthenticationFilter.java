...
import com.distressed.asset.admin.config.shiro.ShiroUtils;
...
public class RememberAuthenticationFilter extends FormAuthenticationFilter {
    ...
    protected boolean isAccessAllowed(ServletRequest request, ServletResponse response, Object mappedValue) {
        ...
        //判断是否需要刷新管理员缓存信息
        AdminUserDTO loginUser = (AdminUserDTO) subject.getPrincipal();
        if (loginUser != null) {
            Object cacheInfo = redisService.get(ShiroUtils.ONLINE_UPDATE_SESSION_USER_INFO + loginUser.getId());
            if (cacheInfo != null) {
                AdminUserDTO cacheUser = (AdminUserDTO) cacheInfo;
